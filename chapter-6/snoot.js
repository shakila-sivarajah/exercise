...
function updateDays(){
	...
	while(dates[28]){
		//remove child with index of 28 until this index is is empty
...
function validateAddress(fieldsetId){
	...
	try {
		...
		// validate state select elementCount
		...
		if (fieldsetValidity === false){
			// throw appropriate message based on current fieldsetId
...
function validatePayment(){
	...
	try {
		...
		if (ccNumElement.value === ""){
			...
			ccNumElement.style.background = "rbg(255,233,233)";
			fieldsetValidity = false;
		} else {
	...
	try {
		...
		if((usernameElement.value !== "" && pass1Element.value !== "" && pass2Element.value !== "")){
			...
			if (pass1Element.value !== pass2Element.value){
				...
				throw "Password entered do not match; Please reenter."
			}
...
function validateForm(evt){
	if (evt.preventDefault){
		...
		evt.returnValue = false; //prevent form from submittingin IE8
	}
